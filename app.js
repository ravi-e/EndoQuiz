...
// Persistent Data
let userStats = JSON.parse(localStorage.getItem("endoStats")) || {
  totalSolved: 0,
  totalCorrect: 0,
  highScore: 0,
};
...
function updateDBDashboard() {
  ...
  const maxCount =
    allQuestions.length > 0 ? Math.max(...Object.values(dist)) : 0;
...
function startWeakDomainDrill(category) {
  ...
  pool.sort((a, b) => {
    const dataA = srsData[getQuestionId(a)] || {
      repetitions: 0,
      easeFactor: 2.5,
    };
    const dataB = srsData[getQuestionId(b)] || {
      repetitions: 0,
      easeFactor: 2.5,
    };
...
function parseDistractorReasoning(explanation) {
  ...
  if (hasLetters) {
    ...
    letters.forEach((letter) => {
      reasonings[letter] = incorrectPart
        .split("CLINICAL PEARL:")[0]
        .split("REFERENCE:")[0]
        .trim();
    });
  }
...
function loadReviewQuestion() {
  ...
  if (q.explanation) {
    if (q.clinicalPearl) {
      ...
      DOM.review.pearlContainer.classList.add("hidden");
    }
    if (q.reference) {
      const searchUrl = `https://duckduckgo.com/?q=!ducky+endodontics+${encodeURIComponent(
        q.reference,
      ...
      DOM.review.referenceText.classList.add("hidden");
    }
  }
  updateFavoriteUI(q);
}
...
function renderDomainPerformance() {
  ...
  const sortedStats = Object.entries(domainStats).sort(
    (a, b) => a[1].correct / a[1].attempts - b[1].correct / b[1].attempts,
  );
